...
function App() {
  ...
  useEffect(() => {
    fetch('http://localhost:3000/clases').then(res => res.json()).then(data => setClases(data));
  }, []);
  ...
  const handleScan = (result) => {
    if (result && result[0]?.rawValue && escaneando) {
      ...
      fetch('http://localhost:3000/asistencia', {
        ...
        body: JSON.stringify({ alumno_id: result[0].rawValue, clase_id: seleccion.id })
      })
      .then(res => res.json())
      .then(data => {
  ...
  const enviarReporte = () => {
    if(window.confirm("¿Enviar reporte del grupo 6N LIDTS?")) {
      fetch('http://localhost:3000/cerrar-clase', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        ...
        <div style={{ backgroundColor: 'white', color: '#333', padding: '30px', borderRadius: '15px', width: '450px', textAlign: 'center' }}>
          {!seleccion ? (
            <>
              ...
              {clases.map(c => (
                <button key={c.id} onClick={() => setSeleccion(c)} style={{ width: '100%', padding: '12px', margin: '5px 0', cursor: 'pointer', borderRadius: '8px', border: '2px solid #003b70', fontWeight: 'bold', color: '#003b70', background: 'white' }}>
                  {c.nombre}
                </button>
              ))}
            </>
            ...
            <>
              ...
              <div style={{marginTop: '20px', display: 'flex', flexDirection: 'column', gap: '10px'}}>
                <button onClick={enviarReporte} style={{ width: '100%', padding: '15px', backgroundColor: '#28a745', color: 'white', border: 'none', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer' }}>
                  Finalizar Clase y Enviar Reporte
                </button>
                <button onClick={() => {setSeleccion(null); setMensaje(""); setEscaneando(true);}} style={{ color: '#dc3545', cursor: 'pointer', background: 'none', border: 'none' }}>
